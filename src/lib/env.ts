...
export const env = {
  ...
  ENCRYPTION_KEY: process.env.ENCRYPTION_KEY || "fallback-encryption-key-32chars!",
  ...
  IS_DEMO_MODE: !(process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY)
} as const;
...
export function validateEnvironment() {
  if (env.IS_PRODUCTION) {
    ...
    if (missingVars.length > 0) {
      ...
      console.warn("The application will run in demo mode.");
    }
  }
  
  // Log configuration status
  console.log("🔧 Environment Configuration:");
  console.log(`   - Demo Mode: ${env.IS_DEMO_MODE ? "✅" : "❌"}`);
  console.log(`   - Supabase: ${env.ENABLE_SUPABASE ? "✅" : "❌"}`);
  console.log(`   - Analytics: ${env.ENABLE_ANALYTICS ? "✅" : "❌"}`);
  console.log(`   - Maps: ${env.ENABLE_MAPS ? "✅" : "❌"}`);
} 